    ...
    function setPosition(int24 tickLower, int24 tickUpper, int128 liquidityDelta) external {
        ...
        if (flippedLower) {
            if (liquidityDelta < 0) {
                ...
                ticks.clear(tickLower, block.timestamp / 1 weeks);
        ...
        if (flippedUpper) {
            if (liquidityDelta < 0) {
                ...
                ticks.clear(tickUpper, block.timestamp / 1 weeks);
        ...
        while (tick != target) {
            if (tick < target) {
                ...
                if (ticks[tick].liquidityGross > 0)
                    ticks.cross(tick, feeGrowthGlobal0X128, feeGrowthGlobal1X128, 0, 0, uint32(block.timestamp), 0, 0);
                tick--;
            }
        }
    }
