...
contract TestRamsesV3Callee is IUniswapV3MintCallback, IUniswapV3SwapCallback, IUniswapV3FlashCallback {
    ...
    function swapToLowerSqrtPrice(address pool, uint160 sqrtPriceX96, address recipient) external {
        IRamsesV3Pool(pool).swap(recipient, true, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }

    function swapToHigherSqrtPrice(address pool, uint160 sqrtPriceX96, address recipient) external {
        IRamsesV3Pool(pool).swap(recipient, false, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }
    ...
    function mint(
        ...
        IRamsesV3Pool(pool).mint(recipient, index, tickLower, tickUpper, amount, abi.encode(msg.sender));
    }
    ...
    function uniswapV3MintCallback(uint256 amount0Owed, uint256 amount1Owed, bytes calldata data) external override {
        ...
        if (amount0Owed > 0)
            IERC20Minimal(IRamsesV3Pool(msg.sender).token0()).transferFrom(sender, msg.sender, amount0Owed);
        if (amount1Owed > 0)
            IERC20Minimal(IRamsesV3Pool(msg.sender).token1()).transferFrom(sender, msg.sender, amount1Owed);
    }
    ...
    function flash(
        ...
        IRamsesV3Pool(pool).flash(recipient, amount0, amount1, abi.encode(msg.sender, pay0, pay1));
    }
    ...
    function uniswapV3FlashCallback(uint256 fee0, uint256 fee1, bytes calldata data) external override {
        ...
        if (pay0 > 0) IERC20Minimal(IRamsesV3Pool(msg.sender).token0()).transferFrom(sender, msg.sender, pay0);
        if (pay1 > 0) IERC20Minimal(IRamsesV3Pool(msg.sender).token1()).transferFrom(sender, msg.sender, pay1);
    }
