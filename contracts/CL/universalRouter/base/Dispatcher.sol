            ...
            if (command < Commands.SECOND_IF_BOUNDARY) {
                ...
                if (command < Commands.FIRST_IF_BOUNDARY) {
                    ...
                    if (command == Commands.V2_SWAP_EXACT_IN) {
                        ...
                        bytes calldata path = inputs.toBytes(3);
                        (SwapRoute.Route[] memory pathDecoded) = abi.decode(path, (SwapRoute.Route[]));
                        address payer = payerIsUser ? lockedBy : address(this);
                        v2SwapExactInput(map(recipient), amountIn, amountOutMin, pathDecoded, payer);
                        ...
                        (SwapRoute.Route[] memory pathDecoded) = abi.decode(path, (SwapRoute.Route[]));
                        address payer = payerIsUser ? lockedBy : address(this);
                        v2SwapExactOutput(map(recipient), amountOut, amountInMax, pathDecoded, payer);
