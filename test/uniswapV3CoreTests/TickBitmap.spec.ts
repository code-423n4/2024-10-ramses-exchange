...
import { TickBitmapTest } from "./../../typechain-types";
    ...
    describe("#nextInitializedTickWithinOneWord", () => {
        ...
        describe("lte = false", async () => {
            it("returns tick to right if at initialized tick", async () => {
                const { next, initialized } =
                    await tickBitmap.nextInitializedTickWithinOneWord(
                        78,
                        false
                    );
            ...
            it("gas cost on boundary", async () => {
                await snapshotGasCost(
                    await tickBitmap.getGasCostOfNextInitializedTickWithinOneWord(
                        255,
                        false
                    )
                );
        ...
        describe("lte = true", () => {
            it("returns same tick if initialized", async () => {
                const { next, initialized } =
                    await tickBitmap.nextInitializedTickWithinOneWord(78, true);
            ...
            it("returns tick directly to the left of input tick if not initialized", async () => {
                const { next, initialized } =
                    await tickBitmap.nextInitializedTickWithinOneWord(79, true);
            ...
            it("word boundary less 1 (next initialized tick in next word)", async () => {
                const { next, initialized } =
                    await tickBitmap.nextInitializedTickWithinOneWord(72, true);
