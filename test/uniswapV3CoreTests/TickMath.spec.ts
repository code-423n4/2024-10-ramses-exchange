...
describe("TickMath", () => {
    ...
    describe("#getSqrtRatioAtTick", () => {
        it("throws for too low", async () => {
            ...
            ).to.be.revertedWithCustomError(tickMath, "T");
        });
        ...
        for (const absTick of [
            ...
            150_000, 250_000, 500_000, 738_203,
        ]) {
            for (const tick of [-absTick, absTick]) {
                describe(`tick ${tick}`, () => {
                    it("is at most off by 1/100th of a bips", async () => {
                        ...
                        const absDiff = new Decimal(result.toString())
                            .sub(jsResult)
                            .abs();
                        expect(absDiff.div(jsResult).toNumber()).to.be.lt(
                            0.000001,
                        );
                    });
                    it("result", async () => {
                        expect(
                            (
                                await tickMath.getSqrtRatioAtTick(tick)
                            ).toString(),
                        ).to.matchSnapshot();
                    });
                    it("gas", async () => {
    ...
    describe("#getTickAtSqrtRatio", () => {
        it("throws for too low", async () => {
            await expect(
                tickMath.getTickAtSqrtRatio(MIN_SQRT_RATIO - 1n),
            ).to.be.revertedWithCustomError(tickMath, "R");
        });
        ...
        it("ratio of min tick", async () => {
            expect(await tickMath.getTickAtSqrtRatio(MIN_SQRT_RATIO)).to.eq(
                MIN_TICK,
            );
        });
        ...
        it("ratio closest to max tick", async () => {
            expect(
                await tickMath.getTickAtSqrtRatio(MAX_SQRT_RATIO - 1n),
            ).to.eq(MAX_TICK - 1);
        });

        for (const ratio of [
            MIN_SQRT_RATIO,
            encodePriceSqrt(10n ** 12n, 1n).toString(),
            encodePriceSqrt(10n ** 6n, 1n).toString(),
            encodePriceSqrt(1n, 64n).toString(),
            encodePriceSqrt(1n, 8n).toString(),
            encodePriceSqrt(1n, 2n).toString(),
            encodePriceSqrt(1n, 1n).toString(),
            encodePriceSqrt(2n, 1n).toString(),
            encodePriceSqrt(8n, 1n).toString(),
            encodePriceSqrt(64n, 1n).toString(),
            encodePriceSqrt(1n, 10n ** 6n).toString(),
            encodePriceSqrt(1n, 10n ** 12n).toString(),
            MAX_SQRT_RATIO - 1n,
        ]) {
            describe(`ratio ${ratio}`, () => {
                ...
                it("ratio is between the tick and tick+1", async () => {
                    ...
                    const ratioOfTickPlusOne =
                        await tickMath.getSqrtRatioAtTick(tick + 1n);
                    expect(ratio).to.be.gte(ratioOfTick);
                    expect(ratio).to.be.lt(ratioOfTickPlusOne);
                });
                it("result", async () => {
                    expect(
                        await tickMath.getTickAtSqrtRatio(ratio),
                    ).to.matchSnapshot();
                });
                it("gas", async () => {
                    await snapshotGasCost(
                        tickMath.getGasCostOfGetTickAtSqrtRatio(ratio),
                    );
                });
            });
        }
